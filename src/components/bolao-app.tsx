/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars, react-hooks/purity, react-hooks/set-state-in-effect, react-hooks/immutability, @next/next/no-img-element, react/no-unescaped-entities */
'use client';
...
export type AppSection = 'home' | 'matches' | 'results' | 'leaderboard' | 'calendar' | 'history' | 'leagues';
...
const appRoutes: Record<AppSection, string> = {
  home: '/dashboard',
  matches: '/matches',
  results: '/results',
  calendar: '/calendar',
...
function toUserProfile(user: AuthenticatedUser): UserProfile {
  return {
    id: user.id,
...
interface BolaoAppProps {
  initialSection?: AppSection;
  authenticatedUser: AuthenticatedUser;
}

export default function BolaoApp({ initialSection = 'home', authenticatedUser }: BolaoAppProps) {
  const router = useRouter();
  const authenticatedProfile = toUserProfile(authenticatedUser);
  // Estados da Aplicação (admin removido, calendar adicionado)
  const [activeTab, setActiveTab] = useState<AppSection | 'auth'>(initialSection);
  const [matches, setMatches] = useState<Match[]>([]);
  const [users, setUsers] = useState<UserProfile[]>([]);
  const [predictions, setPredictions] = useState<Prediction[]>([]);
  
  // Estados de Autenticação Local
  const [loggedInUser, setLoggedInUser] = useState<AuthenticatedUser>(authenticatedUser);
  ...
  const [selectedUserId, setSelectedUserId] = useState<string>(authenticatedUser.id);
  const [currentUser, setCurrentUser] = useState<UserProfile | null>(authenticatedProfile);
  ...
  useEffect(() => {
    ...
    setCurrentUser(toUserProfile(authenticatedUser));
  ...
  const fetchData = useCallback(async (showLoader = false) => {
    try {
      ...
      // Palpites do usuário
      const predsRes = await fetch(`/api/predictions?leagueId=${selectedLeagueId}`);
      ...
      const visibleMatchIds = visibleMatches.map((m: Match) => m.id);
      if (visibleMatchIds.length > 0) {
        try {
          const statsRes = await fetch(`/api/matches/stats?matchIds=${visibleMatchIds.join(',')}&leagueId=${selectedLeagueId}`);
          if (statsRes.ok) {
            Object.assign(statsMap, await statsRes.json());
          }
        } catch {
  ...
  // A sessão do servidor é a fonte de verdade para o usuário autenticado.
  ...
  const handleShareWhatsApp = () => {
    const text = `🔥 Meu saldo no Bolão Copa dos Crias é de ${currentUser?.points || 0} pontos! Dê seus palpites nos placares reais da Copa de 2026 e venha competir comigo no ranking.`;
          ...
          <div className="d-flex align-items-center gap-3">
            ...
            <div className="d-flex align-items-center bg-dark bg-opacity-40 border border-secondary border-opacity-30 rounded-pill p-1 shadow-sm">
              ...
              <span className="text-white fw-bold text-truncate pe-2" style={{ fontSize: '0.75rem', maxWidth: '140px' }}>
                {authenticatedProfile.image} {authenticatedProfile.name.split(' ')[0]}
              </span>
            </div>
                    ...
                    <div className="col-12 col-lg-8">
                      ...
                      <div className="glass-card p-4 mb-4 text-start border-info border-opacity-25" style={{ background: 'linear-gradient(135deg, rgba(0, 255, 135, 0.05) 0%, rgba(96, 239, 255, 0.1) 100%)' }}>
                        ...
                        <div className="d-flex gap-2 flex-wrap">
                          <Link href={appRoutes.matches} className="btn btn-neon-green px-4 py-2" onClick={() => setActiveTab('matches')}>
                              ...
                              <form onSubmit={handleCreateLeague}>
                                ...
                                <div className="p-3 mb-3 rounded bg-dark bg-opacity-40 border border-secondary border-opacity-20">
                                  ...
                                  <div className="row g-2">
                                    ...
                                    <div className="col-4">
                                      <label className="form-label text-secondary" style={{ fontSize: '0.7rem' }}>Vitória Casa (+)</label>
                                      <input 
                                        type="number" 
                                        className="form-control form-control-sm bg-dark text-white border-secondary"
                                        value={leagueForm.pointsWinnerHome}
                                        onChange={(e) => setLeagueForm({ ...leagueForm, pointsWinnerHome: e.target.value })}
                                        required 
                                      />
                                    </div>
                                    <div className="col-4">
                                      <label className="form-label text-secondary" style={{ fontSize: '0.7rem' }}>Vitória Fora (+)</label>
                                      <input 
                                        type="number" 
                                        className="form-control form-control-sm bg-dark text-white border-secondary"
                                        value={leagueForm.pointsWinnerAway}
                                        onChange={(e) => setLeagueForm({ ...leagueForm, pointsWinnerAway: e.target.value })}
                                        required 
                                      />
                                    </div>
                                    <div className="col-4">
